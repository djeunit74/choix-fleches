...
function chooseCandidate(deal, candidates) {
  ...
  if (deal.shop.includes("archerie.fr")) {
    return firstReasonable || (current == null ? first : current);
  }

  if (deal.shop.includes("erhart-sports.com")) {
    return firstReasonable || (current == null ? first : current);
  }

  if (deal.shop.includes("bourgognearcherie.com")) {
    return firstReasonable || (current == null ? first : current);
  }

  if (deal.shop.includes("archers-delight.com")) {
    return firstReasonable || (current == null ? first : current);
  }

  if (deal.shop.includes("hristo.hr")) {
    return firstReasonable || (current == null ? first : current);
  }

  if (deal.shop.includes("heraclesarcherie.fr")) {
    if (current == null) return first;
    const nearest = sorted.reduce((best, candidate) =>
      Math.abs(candidate - current) < Math.abs(best - current) ? candidate : best, sorted[0]);
    return nearest;
  }
