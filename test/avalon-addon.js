/* Assistant Archer TEST - Avalon enrichi, API runtime stable Pré-alpha v19. */
(() => {
  'use strict';
  const VERSION='Pré-alpha v19';
  ...
  function buildAvalonRecommendation(input){
    ...
    const ranked=rankModels(candidates.map(c=>c.name),input,profile).map(entry=>{const c=map.get(entry.model);return {...entry,advisedSpine:String(c?.spine||spineOf(entry.model)),avalonMappingBasis:c?.basis||'',avalonMappingNote:c?.note||'',manufacturerVerified:true,manufacturerSource:AVALON_CURRENT,catalogAuditKey:c?.family?.key||''};});
    const top=ranked[0]?.meta||null,point=estimatePointSetup(input,top?.pointRange||profile.pointRange,top),direct=ranked.filter(x=>x.avalonMappingBasis==='direct').length;
    return {brand:'avalon',mode:'avalon-table-v19',primary:hit.group,comparisonSpine:ranked[0]?.advisedSpine||null,softer:null,stiffer:null,load:input.drawWeight,confidence:direct?'Moyenne':'Prudente',confidenceReasons:[`Groupe ${hit.group} du tableau Avalon 2016 (${hit.rowLabel}, ${hit.length}\").`,`${direct} correspondance(s) directe(s) Tyro/Classic ; les rapprochements Hybrid/Tyro Shaft restent signalés séparément.`],models:ranked,alternativeModels:[],fallbackLabel:'',recommendedMaterial:'carbon',recommendedDiameter:'thin',recommendedPointRange:top?.pointRange||profile.pointRange,recommendedPointWeight:point.recommended,recommendedPointChoices:point.pointChoices,recommendedPointProfile:point.profile,recommendedPointSofter:point.softerOption,recommendedPointStiffer:point.stifferOption,pointWeightNote:point.note,recommendedSeries:top?.seriesTier||'club',recommendedMass:top?.massClass||'medium',recommendedTolerance:top?.toleranceClass||'standard',recommendedComponentSystem:top?.componentSystem||'insert',recommendedUseCase:top?.useCase||'club',recommendedDistanceBand:top?.distanceBand||'mixed',notes:['Tyro et Classic : correspondance directe du selector Avalon 2016.','Hybrid et Tyro Shaft : rapprochement par spine actuel, sans prétendre à une continuité historique certifiée.']};
  }
  function installCatalog(){const select=document.getElementById('preferredBrand');if(select&&!select.querySelector('option[value="avalon"]')){const o=document.createElement('option');o.value='avalon';o.textContent='Avalon';select.appendChild(o);}if(typeof BRAND_ORDER!=='undefined'&&!BRAND_ORDER.includes('avalon'))BRAND_ORDER.push('avalon');const catalog={};for(const f of Object.values(CURRENT))for(const spine of f.spines){const name=`${f.label} ${spine}`;(catalog[String(spine)]??=[]).push(name);if(typeof catalogState!=='undefined'&&typeof normalizeModelKey==='function')catalogState.models[normalizeModelKey(name)]=modelMeta(f,spine);}if(typeof arrowCatalog!=='undefined')arrowCatalog.avalon=catalog;if(typeof catalogState!=='undefined')catalogState.catalog.avalon=catalog;}
  ...
  function installLegacyWrapper(){if(window.__avalonWrappedV19||typeof buildBrandRecommendation!=='function')return;const original=buildBrandRecommendation;buildBrandRecommendation=function(input,brand){if(brand==='avalon')return buildAvalonRecommendation(input)||original(input,brand);return original(input,brand);};window.__avalonWrappedV19=true;}
  function install(){try{installCatalog();exposeApi();installLegacyWrapper();const release=document.getElementById('appReleaseStatic');if(release)release.textContent=`Version : ${VERSION}`;}catch(e){console.warn('Avalon v19 non chargé',e);}}
