...
(() => {
  ...
  const waManual='https://extranet.worldarchery.sport/documents/index.php/Coaches/Accreditation/Coaching_Levels/Coaching_Manual_Level2.pdf';
  const waCoaching='https://www.worldarchery.sport/sport/education/coaching';
  const hoytSupport='https://hoyt.com/pages/target-recurve-safety-and-warnings';
  const sourceLinks=`<p class="muted"><strong>Sources :</strong> <a href="${waManual}" target="_blank" rel="noopener">World Archery — Coach's Manual Level 2</a> · <a href="${waCoaching}" target="_blank" rel="noopener">World Archery — ressources coaching</a> · <a href="${hoytSupport}" target="_blank" rel="noopener">Hoyt — Recurve Riser Setup & Limb Installation</a></p>`;
  ...
  function installPublishButton(){
    ...
    const help=document.createElement('p'); help.style.cssText='margin:.5rem 0 0;font-size:.85em'; help.textContent='GitHub s ouvre : clique sur Run workflow pour confirmer la publication.';
  ...
  function installAuditNotes() {
    ...
    addDetails('[data-panel="arc-setup"]','expertBaseAudit','1. Priorites de reglage',`<p><strong>Ordre conseille :</strong> commencer par le montage et les alignements mecaniques. Regler ensuite le tiller et le band, puis le point d encochage, le centrage/berger et enfin valider au tir.</p><p>Si un probleme d alignement est visible, ne cherchez pas a le masquer avec le berger ou un autre reglage fin.</p>${sourceLinks}`,'#arcSetupResult');

    addDetails('[data-panel="arc-setup"]','alignmentGuidedCheck','2. Alignements et montage de base',`
      ...
      <p><strong>5. Une seule correction a la fois.</strong> Corrigez d abord l alignement mecanique. Ensuite seulement, utilisez le berger et les tests au tir pour affiner le centrage.</p>${sourceLinks}`,'#expertBaseAudit');

    addDetails('[data-panel="arc-setup"]','tillerGuidedTest','3. Tiller : reglage et test',`
      ...
      <p><strong>3. Observez.</strong> Notez la stabilite de visee, les vibrations et surtout une eventuelle sensation de bascule de la poignee dans la main.</p>
      ...
      <p><strong>6. Validez.</strong> Gardez le reglage qui donne le comportement le plus regulier sur plusieurs series. Si aucune amelioration nette n apparait, revenez au reglage de reference.</p>${sourceLinks}`,'#alignmentGuidedCheck');
